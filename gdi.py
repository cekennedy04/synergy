...
GDI9 = GdiFeatureSet(
    ...
    disabled_reason=(
        ...
        "among the worst-affected. Measured over all six sessions, our exports "
        "differ from the cohort by +10.27 deg on pelvis_tilt. Residual "
        "differences are not confined to the pelvis -- hip_flexion runs "
        "-10.57 deg and fpa +4.78, summing to 20.0 deg across the six "
        "non-pelvis variables -- so reduced6 is a smaller exposure rather than "
        "immunity. gdi9 is disabled rather than reduced6 because it adds the "
        "pelvis terms on top with no offsetting benefit, and because the "
        "project's reported numbers all come through reduced6. Note that "
        "these residual differences do NOT produce a score deficit: the "
        "cohort means 93.45 +/- 7.44 on reduced6, within one SD of the "
        "normative 100 +/- 10, and an earlier claim of a uniform ~20-point "
        "offset was retracted -- see section 13 of "
        "docs/2026-08-31-gdi-vs-ucm-audit.md. This is a coordinate-convention "
        "problem, not a trimming one: auto-trimming recovers clean gait "
        "cycles, it does not recalibrate a frame."
    ),
)
